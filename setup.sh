...
check_optional "NEXT_PUBLIC_APP_URL" "auto-detected on Replit, required on Vercel/Railway"
check_optional "INNGEST_DEV" "set to 1 for local dev (no Inngest cloud account needed)"
check_optional "INNGEST_API_KEY" "needed for production Inngest (cloud)"
...
echo ""
echo -e "${BOLD}Auth Setup:${RESET}"
echo ""
echo -e "  Admin login: ${CYAN}admin@example.com${RESET} / ${CYAN}admin123${RESET}"
echo -e "  Generate AUTH_SECRET: ${CYAN}openssl rand -base64 32${RESET}"
echo ""
echo -e "${BOLD}Inngest Setup (background job processing):${RESET}"
echo ""
echo -e "  Dev mode (no account needed):"
echo -e "    Set ${CYAN}INNGEST_DEV=1${RESET} in your .env"
echo -e "    Run: ${CYAN}npx inngest-cli@latest dev -u http://localhost:3000/api/inngest --no-discovery${RESET}"
echo ""
echo -e "  Production mode:"
echo -e "    Get API key from ${CYAN}https://inngest.com${RESET}"
echo -e "    Set ${CYAN}INNGEST_API_KEY${RESET} and ${CYAN}INNGEST_SIGNING_KEY${RESET} in your .env"
echo ""
