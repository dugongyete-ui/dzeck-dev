...
- **Cohere AI integration (ACTIVE)**: AI provider switched from Pollinations to Cohere AI (`cohere-ai` SDK v8). Uses `command-a-03-2025` model via `CohereClient` (replaced `command-r-plus` which was removed by Cohere on Sept 15, 2025). Requires `COHERE_API_KEY` environment variable. Agent loop uses Cohere chat history format (`USER`/`CHATBOT` roles) with native tool calling via `parameterDefinitions`.
